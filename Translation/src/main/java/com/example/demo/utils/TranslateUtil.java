...
public class TranslateUtil {
    ...
    private static String appid="20181109000232117";

    private static String securityKey="3EFPPH11mL8BxIOVebXD";
    ...
    public static String getTransResult(String query, String from, String to ) throws IOException {
        ...
        if (query.length() >= 2000) {
            //post 请求方式
            jsonObject = HttpClient.doPostStr(TRANS_API_HOST, params);
        } else {
            //  get请求方式
            String url = getUrlWithQueryString(TRANS_API_HOST, params);
            jsonObject = HttpClient.doGetStr(url);
        }
        if (jsonObject.get("error_code")!=null) {
            return "翻译" +query+
                    "失败，原因："+jsonObject.get("error_msg");
        }else{
            return jsonObject.getJSONArray("trans_result").getJSONObject(0).getString("dst").toString();
        }
    }
